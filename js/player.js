        ...
        this.attack = (step, level) => {
            ...
            if (this.action === "jabAttack1") {
                ...
                if (this.actionFrame === 12) {
                    ...
                    level.actors.set(("jabAttack3" + this.name).toLowerCase(), new Hitbox("JabAttack3" + this.name, this.pos.plus(new Vector2D(xPos, 0)), new Vector2D(1, 2), "null", true, 8, this, "enemy"));
                }
                ...
                if (this.actionFrame === 12) {
                    ...
                    level.actors.set(("aerialAttack" + this.name).toLowerCase(), new Hitbox("AerialAttack" + this.name, this.pos.plus(new Vector2D(xPos, 0)), new Vector2D(1.25, 2), "null", true, 8, this, "enemy"));
                }
                if (this.actionFrame === 24) {
                    this.action = null;
                }
        ...
        this.act = (step, level, keys) => {
            ...
            if (this.status === null && actor && ((actor instanceof Enemy && actor.status === null) || actor instanceof Projectile)) {
                if (actor instanceof Projectile || !(this.action === "jabAttack1" || this.action === "jabAttack2" || this.action === "jabAttack3" || this.action === "aerialAttack" || this.action === "landingAttack" &&
                    ...
                    this.actionFrame = 0;
                    this.health--;
                    level.vfx.set(("vfx1" + level.frame).toLowerCase(), new Vfx("vfx1" + level.frame, this.pos.plus(new Vector2D(this.size.x / 2 - 1, this.size.y / 2 - 1)), new Vector2D(2, 2), this.direction, "vfx1", 16));
                }
            }
