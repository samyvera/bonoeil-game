		...
		for (let x = 0; x < this.size.x; x++) {
			for (let y = 0; y < this.size.y; y++) {
				for (let i = 0; i < plan.length; i++) {
					...
					else if (ch === 'h') { fieldType = "wood-left"; size = new Vector2D(1, 1); }
					else if (ch === 'i') { fieldType = "wood"; size = new Vector2D(1, 0.5); }
					else if (ch === 'j') { fieldType = "wood-right"; size = new Vector2D(1, 1); }
