class CanvasDisplay {
	...
	public drawVfx = (): void => {
		this.level.vfx.forEach((vfx: Vfx) => {
			var width: number = vfx.size.x * scale;
			var height: number = vfx.size.y * scale;
			var posX: number = (vfx.pos.x - this.viewport.left) * scale;
			var posY: number = (vfx.pos.y - this.viewport.top) * scale;
